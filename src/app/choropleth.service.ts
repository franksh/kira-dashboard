...
import {points, polygon, Polygon, FeatureCollection, GeoJSONObject} from '@turf/helpers';
...
export class ChoroplethService {
  ...
  constructor(private displaycontrolservice: DisplayControlService) {
		this.displaytimesub = combineLatest(displaycontrolservice.displaytime$, this.allchoroplethdists$)
			.subscribe(([time, allchoroplethdists]) => {
				var chororpleth: Choropleth = {distdata: allchoroplethdists[time.timestamp], maximum: this.maxvaluechoroplethdists}
				this.selectedchoroplethdistSource.next(chororpleth)
		})
		this.displaytimesub = combineLatest(displaycontrolservice.displaytime$, this.allchoroplethhosp$)
			.subscribe(([time, allchoroplethhosp]) => {
				var chororpleth: Choropleth = {distdata: allchoroplethhosp[time.timestamp], maximum: this.maxvaluechoroplethhosp}
	...
	computeChoroplethHosp(hospitalnames: string[], voronoihospitals: any, snapshot: SimSnapshot): Choropleth {
		var choropleth: Choropleth = {distdata: [], maximum: 0};
	...
	initChoropethHosp() {
		...
		for (var i in this.simresult.snapshots) {
			var snapshot: SimSnapshot = this.simresult.snapshots[i];
			var ts = snapshot.timestamp;
			var choropleth = this.computeChoroplethHosp(hospitalnames, this.voronoihospitals, snapshot);
