...
export class MapDisplayComponent implements AfterViewInit {
  ...
  basemaplayer = L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoibWFwYm94IiwiYSI6ImNpejY4NXVycTA2emYycXBndHRqcmZ3N3gifQ.rJcFIG214AriISLbB6B5aw', {
		  maxZoom: 18,
  ...
  constructor(private displaycontrolservice: DisplayControlService, public choroplethservice: ChoroplethService, private zone: NgZone) {
    ...
    this.displaycontrolservice.choroplethmode$.subscribe(state =>  {
      state == 'district' ?  this.choroplethmode_isdistrict = true : this.choroplethmode_isdistrict = false;
      this.updateLayers()
    });
    this.displaycontrolservice.selectactive$.subscribe(state => {
      ...
      this.updateLayers();
		...
		this.selectedsnapshotsub = displaycontrolservice.selectedsnapshot$
		    .subscribe(selectedsnapshot => {
		    	this.changeHeatmap(selectedsnapshot.points);
          this.updateLayers();
		  })
    
    this.districtchoroplethsub = choroplethservice.selectedchoroplethdist$
      .subscribe((choropleth: Choropleth) => {
        ...
        districtsgeojson.eachLayer((layer) => {
              ...
              var choroplethvalue = distdata.find(datum => datum.district == districtname).cases
              layer.options.opacity = 0.0
              layer.options.color = "#000000"
              layer.options.fillOpacity = 0.8 * choroplethvalue / choropleth.maximum
            this.distchorolayers.push(layer)
        })
      this.updateLayers()
      })
    this.hospitalchoroplethsub = choroplethservice.selectedchoroplethhosp$
      .subscribe((choropleth: Choropleth) => {
        ...
        hospitalgeojson.eachLayer((layer) => {
              ...
              var distdata: any[] = choropleth.distdata;
              var choroplethvalue = distdata.find(datum => datum.district == hospitalname).cases
              layer.options.opacity = 0.0
              layer.options.color = "#000000"
              layer.options.fillOpacity = 0.8 * choroplethvalue / choropleth.maximum
      ...
      var districtsgeojson = L.geoJSON(DISTRICTSDATA as any);
			districtsgeojson.eachLayer((layer) => {
        layer.options.color = DISTRICT_COLORS[layer.feature.properties.spatial_alias]
        layer.options.fillOpacity = 0
        layer.options.opacity = 0.0
				if (_.find(this.selecteddistricts,(d) => d == layer.feature.properties.spatial_alias) === undefined){
					layer.on({click: (e) => {
						this.zone.run(() => {
					...
					layer.on({click: (e) => {
						this.zone.run(() => {
							var selecteddistricts_copy = _.cloneDeep(this.selecteddistricts)
							_.remove(selecteddistricts_copy, (x) => x == layer.feature.properties.spatial_alias);
							this.displaycontrolservice.changeSelectedDistricts(selecteddistricts_copy)
						})
					}});
    ...
    displaycontrolservice.selectedhospitals$.subscribe(hospitals => {
      ...
      hospitalgeojson.eachLayer((layer) => {
        ...
        layer.options.fillOpacity = 0
        layer.options.opacity = 0.0
        if (_.find(this.selectedhospitals,(d) => d == layer.feature.properties.name) === undefined){
          layer.on({click: (e) => {
            this.zone.run(() => {
              var selectedhospitals_copy = _.cloneDeep(this.selectedhospitals)
              selectedhospitals_copy.push(e.target.feature.properties.name)
              this.displaycontrolservice.changeSelectedHospitals(selectedhospitals_copy)
            })
          }});
        }
        else {
          layer.on({click: (e) => {
            this.zone.run(() => {
              var selectedhospitals_copy = _.cloneDeep(this.selectedhospitals)
              _.remove(selectedhospitals_copy, (x) => x == layer.feature.properties.name);
              this.displaycontrolservice.changeSelectedHospitals(selectedhospitals_copy)
            })
          }});
