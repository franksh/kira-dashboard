      ...
      .subscribe(([choropleths, districts]) => {
        ...
        for (var i in districts) {
          for (var j in this.simresult.snapshots) {
            ...
            y[j] += choropleths[ts].find(data => data.district == districts[i]).cases;
            x[j] = ts;
          }
        }
        this.graph.datadist = [{ x: x, y: y, type: 'scatter', mode: 'points', name: districts[i], marker: {color: "#212121"}}];
      })
    ...
    combineLatest(choroplethservice.allchoroplethhosp$, displaycontrolservice.selectedhospitals$)
      .subscribe(([choropleths, hospital]) => {
        ...
        for (var i in hospital) {
          for (var j in this.simresult.snapshots) {
            var ts = this.simresult.snapshots[j].timestamp;
            y[j] += choropleths[ts].find(data => data.district == hospital[i]).cases;
            x[j] = ts;
          }
        }
        this.graph.datahosp = [{ x: x, y: y, type: 'scatter', mode: 'points', name: hospital[i], marker: {color: "#212121"}}];
      })
  }
